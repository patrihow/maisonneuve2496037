...
@section('title', __('forum.add_document'))

@section('content')
<div class="container mt-4">
    <h1>{{ __('forum.add_document') }}</h1>
    ...
    <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="file" name="file" id="file" class="form-control" accept=".pdf,.zip,.doc,.docx" required>
            <div class="form-text">{{ __('forum.allowed_formats') }}</div>
        </div>

        <button type="submit" class="btn btn-success">{{ __('forum.submit') }}</button>
